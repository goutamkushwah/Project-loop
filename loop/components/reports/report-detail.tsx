...
export function ReportDetail({ report, evidenceLinks = true }: ReportDetailProps) {
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Report statistics">
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Previous period: {formatNumber(content.stats.previousTotalFeedback)}
          </p>
        </article>
              ...
              <tbody className="divide-y divide-slate-100">
                {content.sentiment.map((item) => (
                  <tr key={item.sentiment}>
                    ...
                    <td className="py-3 pr-4 text-slate-600">
                      {formatPercent(item.previousPercentage)} ({formatNumber(item.previousCount)})
                    </td>
                  ...
                  <div key={theme.id} className="rounded-2xl border border-slate-200 p-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <span className="text-sm font-bold text-slate-600">
                        {formatNumber(theme.count)} · {formatPercent(theme.percentage)}
                      </span>
                    </div>

                    {insight ? (
                      <p className="mt-3 text-sm leading-6 text-slate-600">{insight.insight}</p>
                    ) : null}

                    {evidenceLinks ? (
                      <Link
                        href={`/inbox?themeId=${encodeURIComponent(theme.id)}&dateFrom=${content.period.dateFrom}&dateTo=${content.period.dateTo}`}
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          {notableQuotes.map((item) => (
            ...
            >
              ...
              {evidenceLinks ? (
                <Link
                  href={`/inbox?search=${encodeURIComponent(item.content.slice(0, 80))}`}
