...
import type { VoiceOfCustomerReportContent } from "@/types/report";
...
export function ReportDetail({
  ...
  const evidenceById = new Map(
    content.evidence.map((item) => [item.feedbackId, item]),
  );

  const themeById = new Map(
    content.topThemes.map((theme) => [theme.id, theme]),
  );
      ...
      {report.id ? (
        <section className="flex flex-wrap items-center justify-end gap-3">
          <a
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-loop-900 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-loop-800 focus:outline-none focus:ring-2 focus:ring-loop-500 focus:ring-offset-2"
          >
      ...
      >
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-2 text-3xl font-black text-loop-900">
            {formatNumber(content.stats.totalFeedback)}
          </p>

          <p className="mt-1 text-xs text-slate-500">
            Previous period:{" "}
            {formatNumber(content.stats.previousTotalFeedback)}
          </p>
        </article>
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-2 text-3xl font-black text-loop-900">
            {formatPercent(content.stats.classificationCoverage)}
          </p>

          <p className="mt-1 text-xs text-slate-500">
            {formatNumber(content.stats.classifiedFeedback)} classified items
          </p>
        </article>
        ...
        {content.sentiment
          ...
          .map((item) => (
            ...
            >
              ...
              <p className="mt-1 text-xs text-slate-500">
                {formatDelta(item.deltaPercentagePoints)} vs previous period
              </p>
            </article>
          ))}
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Ranked by real feedback assignments
          </p>
        </article>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-600">
            {content.narrative.sentimentSummary}
          </p>

          <div className="mt-5 overflow-x-auto">
            <table className="w-full min-w-[520px] text-left text-sm">
              <caption className="sr-only">
                Sentiment comparison for the current and previous report
                periods.
              </caption>
              ...
              <tbody className="divide-y divide-slate-100">
                {content.sentiment.map((item) => (
                  <tr key={item.sentiment}>
                    ...
                    <td className="py-3 pr-4 text-slate-600">
                      {formatPercent(item.previousPercentage)} (
        ...
        <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
          ...
          {content.topThemes.length === 0 ? (
            <p className="mt-5 rounded-2xl bg-slate-50 p-5 text-sm leading-6 text-slate-600">
              No stored theme assignments were available in this period.
            </p>
          ) : (
            <div className="mt-5 space-y-4">
              {content.topThemes.map((theme) => {
                const insight = content.narrative.themeInsights.find(
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <span className="text-sm font-bold text-slate-600">
                        ...
                        {formatPercent(theme.percentage)}
                      </span>
                    </div>
                    ...
                    {evidenceLinks ? (
                      <Link
                        href={`/inbox?themeId=${encodeURIComponent(
                          theme.id,
                        )}&dateFrom=${content.period.dateFrom}&dateTo=${content.period.dateTo}`}
                        className="mt-3 inline-flex text-sm font-bold text-loop-700 hover:text-loop-900 focus:outline-none focus:ring-2 focus:ring-loop-500 focus:ring-offset-2"
                      >
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          {notableQuotes.map((item) => (
            ...
            >
              ...
              {evidenceLinks ? (
                <Link
                  href={`/inbox?search=${encodeURIComponent(
                    item.content.slice(0, 80),
                  )}`}
                  className="mt-3 inline-flex text-xs font-bold text-loop-700 hover:text-loop-900 focus:outline-none focus:ring-2 focus:ring-loop-500 focus:ring-offset-2"
                >
                  Open evidence
                </Link>
              ) : null}
          ...
          {content.narrative.recommendedActions.map((action, index) => (
            ...
            >
              <div className="flex items-start gap-4">
                <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-loop-100 text-sm font-black text-loop-800">
                  {index + 1}
                </span>
