...
const foundationItems = [
  "Validated single-entry ingestion",
  "Server-side CSV parsing and row validation",
  "Imported-versus-failed row summary",
  "Tenant-scoped bulk writes",
  "Pending classification state for the AI sprint",
] as const;
        ...
        >
          <div>
            ...
            <p className="mt-7 max-w-2xl text-pretty text-lg leading-8 text-slate-600 sm:text-xl">
              Analysts can add feedback one item at a time or upload a validated CSV, while every
              workspace role can review the isolated feedback record permitted by its server-enforced
              access level.
            </p>
            ...
            <div className="mt-10 flex flex-wrap gap-3">
              <span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-800 ring-1 ring-inset ring-emerald-200">
                ...
                Manual and CSV ingestion ready
              </span>
            ...
            <div className="mt-10 flex flex-wrap gap-3">
              <Link
                href={session?.user ? "/inbox" : "/signup"}
              ...
              >
                {session?.user ? "Open workspace feedback" : "Create your workspace"}
              </Link>
              <a
                href="/api/health"
          ...
          <aside className="rounded-3xl border border-white/80 bg-white/90 p-7 shadow-panel backdrop-blur sm:p-9">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm font-semibold text-loop-600">Feedback checklist</p>
                <h2 className="mt-1 text-2xl font-bold text-loop-900">Validated bulk ingestion</h2>
              </div>
            ...
            <ul className="mt-7 space-y-4" aria-label="Completed feedback capabilities">
            ...
            <div className="mt-8 rounded-2xl bg-loop-900 p-5 text-white">
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-loop-300">
                AI deliberately deferred
              </p>
              <p className="mt-2 text-sm leading-6 text-slate-200">
                Imported rows are stored with a pending classification state. Claude calls, structured
                output, and classification processing remain in the scheduled AI sprint.
              </p>
            </div>
          </aside>
        </section>
