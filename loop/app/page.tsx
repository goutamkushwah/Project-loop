          ...
          <div className="flex items-center gap-3">
            {session?.user ? (
              <>
                ...
                <Link
                  href="/dashboard"
                  className="rounded-xl bg-loop-900 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-loop-800 focus:outline-none focus:ring-2 focus:ring-loop-500 focus:ring-offset-2"
                >
                  Dashboard
                </Link>
              </>
              ...
              <>
                <Link
                  href="/login"
                  className="rounded-xl px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-loop-500 focus:ring-offset-2"
                >
                  Sign In
                </Link>
      ...
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-7xl flex-col px-6 py-10 sm:px-10 lg:px-16">
        ...
        >
          ...
          <div>
            ...
            <p className="mt-7 max-w-2xl text-pretty text-lg leading-8 text-slate-600 sm:text-xl">
              Administrators manage members and roles, analysts operate the
              feedback workflow, and viewers receive read-only access—all inside
              a strictly isolated company workspace.
            </p>
            ...
            <div className="mt-10 flex flex-wrap gap-3">
              {foundationItems.map((item) => (
                <span
                  key={item}
                  className="inline-flex items-center gap-2 rounded-full border border-loop-200 bg-white/70 px-4 py-2 text-xs font-semibold text-loop-800 shadow-sm"
                >
                  <span className="h-1.5 w-1.5 rounded-full bg-loop-500" />
                  {item}
                </span>
              ))}
            </div>
          ...
          <div className="relative">
            <div
              aria-hidden="true"
              className="absolute -inset-6 -z-10 rounded-[2rem] bg-gradient-to-br from-loop-200 via-violet-100 to-transparent blur-2xl"
            />

            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-panel">
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                <p className="text-sm font-bold text-loop-900">
                  Workspace roles
                </p>
                <span className="rounded-full bg-loop-50 px-2.5 py-1 text-xs font-semibold text-loop-700">
                  Live
                </span>
              </div>
              ...
              <ul className="mt-5 space-y-4">
                <li className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Full workspace and member control
                    </p>
                  </div>
                  <span className="rounded-full bg-loop-900 px-3 py-1 text-xs font-bold text-white">
                    Full access
                  </span>
                </li>
                ...
                <li className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Runs the feedback workflow end to end
                    </p>
                  </div>
                  <span className="rounded-full bg-loop-100 px-3 py-1 text-xs font-bold text-loop-800">
                    Operate
                  </span>
                </li>
              ...
              <div className="mt-6 rounded-xl bg-slate-50 p-4">
                ...
                <p className="mt-1 text-sm text-slate-700">
                  Every query is scoped to a workspace, so no company ever
                  sees another company&apos;s data.
                </p>
              </div>
            </div>
          </div>
        ...
        <footer className="flex flex-col items-center justify-between gap-4 border-t border-slate-200 py-8 text-sm text-slate-500 sm:flex-row">
          ...
          <div className="flex items-center gap-6">
            <a href="#about" className="transition hover:text-loop-800">
              About
            </a>
            <a href="/api/health" className="transition hover:text-loop-800">
              System health
            </a>
