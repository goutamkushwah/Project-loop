  ...
  return (
    <main className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-14">
      <div className="flex flex-col gap-5 border-b border-slate-200 pb-8 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-base leading-7 text-slate-600">
            Volume, stored sentiment, and theme assignments are calculated from
            real records in the isolated {user.workspace.name} workspace.
            Every chart uses one server-validated filter set.
          </p>
        </div>
      ...
      {!parsedQuery.success ? (
        ...
        >
          The requested dashboard filters were invalid, so LOOP restored the
          default 30-day view.
        </div>
      ) : null}
      ...
      <section className="mt-6 rounded-3xl border border-slate-200 bg-slate-50 p-5 sm:p-7">
        ...
        <p className="mt-3 max-w-4xl text-sm leading-6 text-slate-600">
          Volume and stat totals come directly from PostgreSQL. Sentiment and
          theme charts remain in intentional empty states until the scheduled
          AI classification and clustering work stores real results during Week
          3.
        </p>
      </section>
    </main>
