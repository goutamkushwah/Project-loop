...
export default async function ReportsPage({ searchParams }: ReportsPageProps) {
  ...
  const parsedQuery = reportListQuerySchema.safeParse({
    ...
    periodFrom: first(searchParams?.periodFrom),
    periodTo: first(searchParams?.periodTo),
  ...
  return (
    <main className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-14">
      <div className="flex flex-col gap-5 border-b border-slate-200 pb-8 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-sm font-bold uppercase tracking-[0.2em] text-loop-600">Week 4 · Day 16</p>
          <h1 className="mt-3 text-4xl font-black tracking-tight text-loop-900">Voice-of-Customer reports</h1>
          <p className="mt-3 max-w-3xl text-base leading-7 text-slate-600">
            Turn a chosen period of {user.workspace.name} feedback into a saved, evidence-backed leadership digest with real theme counts, sentiment shifts, verbatim customer feedback, and recommended actions.
          </p>
        </div>
        <span className="w-fit rounded-full bg-loop-100 px-4 py-2 text-sm font-bold text-loop-800">{user.role}</span>
      </div>
      ...
      {!parsedQuery.success ? (
        <div role="alert" className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-medium text-amber-900">
          The requested report-list filters were invalid, so LOOP restored the default saved-report view.
        </div>
      ) : null}
      ...
      <div className="mt-6"><ReportList page={reports} /></div>
    </main>
  );
}