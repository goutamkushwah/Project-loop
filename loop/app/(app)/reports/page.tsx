...
export default async function ReportsPage({
  ...
  const parsedQuery = reportListQuerySchema.safeParse({
    ...
    pageSize: first(resolvedSearchParams?.pageSize),
    search: first(resolvedSearchParams?.search),
    periodFrom: first(resolvedSearchParams?.periodFrom),
    periodTo: first(resolvedSearchParams?.periodTo),
    sortBy: first(resolvedSearchParams?.sortBy),
    sortOrder: first(resolvedSearchParams?.sortOrder),
  });
  ...
  return (
    <main className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-14">
      <div className="flex flex-col gap-5 border-b border-slate-200 pb-8 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-base leading-7 text-slate-600">
            Turn a chosen period of {user.workspace.name} feedback into a
            saved, evidence-backed leadership digest with real theme counts,
            sentiment shifts, verbatim customer feedback, and recommended
            actions.
          </p>
        </div>
      ...
      {!parsedQuery.success ? (
        ...
        >
          The requested report-list filters were invalid, so LOOP restored the
          default saved-report view.
        </div>
      ) : null}
      ...
      <div className="mt-8">
        {canGenerate ? (
          ...
          <div className="rounded-2xl border border-slate-200 bg-white px-5 py-4 text-sm leading-6 text-slate-600 shadow-sm">
            Your Viewer role can read saved reports but cannot generate new
            reports.
          </div>
        )}
      </div>
